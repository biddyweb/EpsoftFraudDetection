raw_data1 <- read.csv('data//高血压住院药品-完整版1.txt',header=T,sep='\t')
setdiff(colnames(raw_data1),'住院天数')

raw_data <- read.csv('data//门诊高血压1.txt',header=T)
colnames(raw_data)<-setdiff(colnames(raw_data1),'住院天数')
colnames(raw_data)[9] <- '总费用'

library(arules)
library(ggplot2)
p2 <- ggplot(raw_data,aes(x=年龄,y=总费用))
print(p2+geom_point(position = "jitter",aes(color=在职.退休))+opts(title='年龄与总费用关系',plot.title=theme_text(face="bold",size=10,hjust=0.5,vjust=2,colour="black"))+stat_smooth())+y 

p3 <- ggplot(raw_data,aes(x=身份类别,y=总费用))
...
p4 <- ggplot(raw_data,aes(x=单位性质,y=总费用))
...
p5 <- ggplot(raw_data,aes(x=单位经济类型,y=总费用))
...
p6 <- ggplot(raw_data,aes(x=医院等级,y=总费用))
...
train_list <- apply(drugs11,1,function(x){unlist(strsplit(x,','))})

names(train_list) <- paste("Tr",c(1:length(train_list)), sep = "")
trans <- as(train_list, "transactions")
trans
summary(trans)
